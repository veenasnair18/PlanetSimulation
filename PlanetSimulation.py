...
class Planet:
    ...
    def __init__(self, x, y, radius, color, mass, scale_factor=1.0):
        self.x = x  
        self.y = y  
        self.radius = radius
        self.color = color
...
def main():
    ...
    Mercury = Planet(0.387 * Planet.AstroUnit, 0, 4, (112, 128, 144), 3.285 * 10**23, scale_factor=1.5)
    Mercury.y_val = -47.4 * 1000  

    Venus = Planet(0.72 * Planet.AstroUnit, 0, 6, (255, 69, 0), 4.87 * 10**24, scale_factor=1.5)
    Venus.y_val = -35.02 * 1000

    Earth = Planet(-1 * Planet.AstroUnit, 0, 8, (30, 144, 255), 5.9742 * 10**24, scale_factor=1.5)
    Earth.y_val = 29.783 * 1000

    Mars = Planet(-1.524 * Planet.AstroUnit, 0, 6, (178, 34, 34), 6.39 * 10**23, scale_factor=1.5)
    Mars.y_val = 24.077 * 1000

    Jupiter = Planet(5.20 * Planet.AstroUnit, 0, 12, (197, 179, 88), 1.898 * 10**27, scale_factor=0.5)
    Jupiter.y_val = 13.07 * 1000

    Saturn = Planet(9.58 * Planet.AstroUnit, 0, 10, (238, 232, 170), 5.68 * 10**26, scale_factor=0.3)
    Saturn.y_val = 9.69 * 1000

    Uranus = Planet(19.18 * Planet.AstroUnit, 0, 8, (95, 158, 160), 8.68 * 10**25, scale_factor=0.2)
    Uranus.y_val = 6.81 * 1000

    Neptune = Planet(30.07 * Planet.AstroUnit, 0, 8, (0, 0, 255), 1.02 * 10**26, scale_factor=0.1)
